...
const GoogleErrorBanner = ({ error }) => error ? (
    <div className="p-4 bg-red-50 text-red-700 text-sm rounded-2xl border border-red-100">
        <div className="flex items-start gap-2 mb-2">
            <AlertCircle className="w-4 h-4 shrink-0 mt-0.5 text-red-500" />
            <span className="font-semibold">No account found for this Google email.</span>
        </div>
        <p className="text-red-500 pl-6 text-[13px] leading-relaxed">
            You must <Link to="/register" className="font-bold text-blue-600 hover:underline">sign up first</Link> before you can sign in with Google.
        </p>
    </div>
) : null;
...
const Login = () => {
    ...
    const handleGoogleSuccess = async (credentialResponse) => {
        ...
        if (result.success) {
            ...
            if (result.message?.toLowerCase().includes('no account found')) {
                setGoogleError(true);
            } else {
                setError(result.message);
            }
                        ...
                        <div className="w-full max-w-[420px]">
                            ...
                            {view === 'social' && (
                                <div className="space-y-4">
                                    <GoogleErrorBanner error={googleError} />
                                    ...
                                    <button
                                        onClick={() => { setView('otp-email'); setError(''); setGoogleError(false); }}
                                    ...
                                    <div className="text-center pt-2">
                                        <button
                                            onClick={() => { setView('password'); setError(''); setGoogleError(false); }}
                ...
                {view === 'social' && (
                    <div className="flex-1 flex flex-col items-center px-6 animate-in slide-in-from-left-4 fade-in duration-300">
                        ...
                        <div className="w-full mb-4 space-y-2">
                            <GoogleErrorBanner error={googleError} />
                            {error && (
                                <div className="w-full p-3 bg-red-50 text-red-600 text-sm rounded-xl border border-red-100 flex items-start gap-2">
                                    <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
                                    {error}
                                </div>
                            )}
